...
public class JavaEntitySpliterator extends JavaBaseVisitor<Entity>implements EntitySpliterator {
  ...
  public Entity visitSingleStaticImportDeclaration(final SingleStaticImportDeclarationContext ctx) {
    ...
    return AntlrHelper.fromParseRuleContext(ctx, tokenStream).type(JavaEntityType.IMPORT.name())
        // typeName + "." + Identifier
        .value(AntlrHelper.getTextFromRangeOfChild(ctx, 2, 4)).property("static", true)
  ...
  public Entity visitSingleTypeImportDeclaration(final SingleTypeImportDeclarationContext ctx) {
    ...
    return AntlrHelper.fromParseRuleContext(ctx, tokenStream).type(JavaEntityType.IMPORT.name())
        .value(ctx.getChild(1).getText()).property("static", false).property("star", false).build();
  }
  ...
  public Entity visitTypeImportOnDemandDeclaration(final TypeImportOnDemandDeclarationContext ctx) {
    ...
    return AntlrHelper.fromParseRuleContext(ctx, tokenStream).type(JavaEntityType.IMPORT.name())
        .value(AntlrHelper.getTextFromRangeOfChild(ctx, 1, 3)).property("static", false)
        .property("star", true).build();
  }
  ...
  public Entity visitStaticImportOnDemandDeclaration(
    ...
    return AntlrHelper.fromParseRuleContext(ctx, tokenStream).type(JavaEntityType.IMPORT.name())
        .value(AntlrHelper.getTextFromRangeOfChild(ctx, 2, 4)).property("static", false)
        .property("star", true).build();
  }
  ...
  public Entity visitPackageDeclaration(final PackageDeclarationContext ctx) {
    ...
    final String value =
        AntlrHelper.getTextFromRangeOfChild(ctx, foundIndex + 1, ctx.getChildCount() - 2);

    final Entity entity = AntlrHelper.fromParseRuleContext(ctx, tokenStream)
        .type(JavaEntityType.PACKAGE.name()).value(value).build();
  ...
  public Entity visitSingleElementAnnotation(final SingleElementAnnotationContext ctx) {
    ...
    return AntlrHelper.fromParseRuleContext(ctx, tokenStream).type(JavaEntityType.ANNOTATION.name())
        .value(ctx.getChild(1).getText()).build();
  }
  ...
  @Override
  public Entity visitNormalAnnotation(NormalAnnotationContext ctx) {
